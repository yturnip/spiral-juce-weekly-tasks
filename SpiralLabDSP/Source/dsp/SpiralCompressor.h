/*
    ...
    (Dynamic Range Compression example from Chapter 7: Dynamic Processing)
*/
...
public:
    ...
    float computeGain (float input)
    {
        ...
        if (x_l_dB > yL_prev_dB)
            y_l_dB = alphaAttack * yL_prev_dB + (1 - alphaAttack) * x_l_dB;
        else
            y_l_dB = alphaRelease * yL_prev_dB + (1 - alphaRelease) * x_l_dB;
